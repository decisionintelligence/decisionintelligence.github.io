<template>
  <q-page padding>
    ...
    <div v-if="data" class="md:tw-w-2/3 md:tw-m-auto">
      <div>
        <div
          class="tw-text-2xl tw-mt-10 tw-leading-10 tw-p-4 tw-my-4 tw-bg-gray-50 tw-border-l-4 tw-border-gray-300"
        >
        ...
        <p class="tw-m-5">
          <svg
            aria-hidden="true"
            class="tw-w-10 tw-h-10 tw-text-gray-400 dark:text-gray-600"
            viewBox="0 0 24 27"
            fill="none"
        ...
        <div v-for="(sub, sub_idx) in data.content" :key="sub_idx + 454541">
          ...
          <!--tw-border-l-4 tw-pl-5 tw-border-l-blue-500 -->
          <div
            class="tw-p-3"
            v-for="(sub_item, idx) in sub.content"
            :key="idx + sub_idx + 1 + 141452"
          >
            <div class="tw-m-2 tw-pl-2">
              <p class="text-weight-bold">{{ sub_item.title }}</p>
              <ul class="tw-list-disc">
                <li
                  v-for="(paper, i) in sub_item.papers"
                  :key="idx * (i + 100)"
                >
                  {{ paper }}
                </li>
