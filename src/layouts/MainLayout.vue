  ...
  <q-layout view="hHh lpR fff">
    ...
    <q-header
      ...
      <q-tabs align="center" v-if="screen.width > 640">
        ...
        <router-link to="/project" class="tw-m-4 tw-text-gray-400  hover:tw-text-blue-500  tw-h-8 tw-text-center"
          active-class=" tw-border-b-2 active-text-color tw-text-sky-600 tw-border-blue-500">
          {{ $t('ProjectPage') }}
        </router-link>
          ...
          <q-popup-proxy ref="pop" class=" tw-w-23 tw-mt-2">
            <div class="tw-p-1">
              <ul class=" tw-space-y-2 tw-text-gray-500 tw-list-inside dark:tw-text-gray-400">
                <li class="tw-pl-3 tw-flex tw-items-center tw-cursor-pointer"
                  :class="[locale === 'en' ? 'active-text-color tw-underline' : '']" @click="hangleClick('en')">
                  English
                </li>
                <li class="tw-pl-3 tw-flex tw-items-center tw-cursor-pointer" @click="hangleClick('zh-CN')"
                  :class="[locale === 'zh-CN' ? 'active-text-color tw-underline' : '']">
                  简体中文
                </li>
              </ul>
            </div>
